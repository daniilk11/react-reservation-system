        ...
        if (Object.keys(validationErrors).length === 0) {
            const payload = {
                ...
                club_member_rules: {
                    ...
                    in_advance_hours: formData.club_in_advance_hours || 0,
                    in_advance_minutes: formData.club_in_advance_minutes || 0,
                    in_advance_day: formData.club_in_advance_day || 0,
                },
                active_member_rules: {
                    night_time: formData.active_night_time || [],
                    reservation_more_24_hours: formData.active_reservation_more_24_hours || [],
                    in_advance_hours: formData.active_in_advance_hours || 0,
                    in_advance_minutes: formData.active_in_advance_minutes || 0,
                    in_advance_day: formData.active_in_advance_day || 0,
                },
                manager_rules: {
                    night_time: formData.manager_night_time || [],
                    reservation_more_24_hours: formData.manager_reservation_more_24_hours || [],
                    in_advance_hours: formData.manager_in_advance_hours || 0,
                    in_advance_minutes: formData.manager_in_advance_minutes || 0,
                    in_advance_day: formData.manager_in_advance_day || 0,
                },
                username: username,
            };

            axios.post(`${config.domenServer}/api/calendars/create`, payload)
                .then(response => {
                    ...
                    {field.type === 'checkbox' ? (
                        field.options.map(option => (
                            <div key={option.value} className="form-check">
                                ...
                                />
                                <label className="form-check-label" htmlFor={`${field.name}-${option.value}`}>{option.label}</label>
                            </div>
                        ))
                    ) : field.type === 'select' ? (
                        <select
        ...
        <div>
            {isLoggedIn ? (
                <div className="container">
                    <h1 className="my-4 text-center text-white" style={{ background: 'linear-gradient(to right, #00b894, #008e7a)', padding: '20px 0' }}>Create new Calendar</h1>
